...
# generate plot by averaging accross all phenotypes
plot.improvement <- function(directory) {
  ...
  baseline = (cd$u0 + uc$u0 + ms$u0 + t2d$u0) / 4
  curve500 = (cd$u500 + uc$u500 + ms$u500 + t2d$u500) / 4 - baseline
  curve1000 = (cd$u1000 + uc$u1000 + ms$u1000 + t2d$u1000) / 4 - baseline
  curve3000 = (cd$u3000 + uc$u3000 + ms$u3000 + t2d$u3000) / 4 - baseline
  
  ymin <- min(curve500, curve1000, curve3000)
  ymax <- max(curve500, curve1000, curve3000)
  xmax <- max(cd$size)
